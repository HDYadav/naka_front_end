...
import { DELETE_WORKPLACE } from "../../utils/constants";
...
const WorkPlace = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        const response = await fetch(`${DELETE_WORKPLACE}${id}`, {
          method: "DELETE",
          headers: {
  ...
  const data = useMemo(() => positions?.data?.workplace || [], [positions]);
...
export default LayoutHOC(WorkPlace);
