...
import { DELETE_STATE } from "../../utils/constants";
...
const StateList = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        const response = await fetch(`${DELETE_STATE}${id}`, {
          method: "DELETE",
          headers: {
  ...
  const data = useMemo(() => positions?.data?.state || [], [positions]);
...
export default LayoutHOC(StateList);
