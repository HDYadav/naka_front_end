...
import { DELETE_EMP_TYPE } from "../../utils/constants";
import { useSelector } from "react-redux";

const EmploymentType = () => {
  const positions = useeEmpType();
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        const response = await fetch(`${DELETE_EMP_TYPE}${id}`, {
          method: "DELETE",
          headers: {
  ...
  const columns = useMemo(
    () => [
      {
        Header: "Emp Type in English",
        accessor: "name",
        sortType: "alphanumeric", // Set sortType for sorting
      },
      {
        Header: "Emp Type  in Hindi",
        accessor: "emptype_hindi",
        sortType: "alphanumeric", // Set sortType for sorting
      },
      {
        Header: "Emp Type  in Marathi",
        accessor: "emptype_marathi",
        sortType: "alphanumeric", // Set sortType for sorting
      },
      {
        Header: "Emp Type in Punjabi",
        accessor: "emptype_punjabi",
        sortType: "alphanumeric", // Set sortType for sorting
      },
      {
        Header: "Actions",
        ...
        Cell: ({ row }) => (
          <div className="flex space-x-4">
            <Link
              to={`/edit_employment_type/${row.values.id}`}
  ...
  const data = useMemo(
    () => positions?.data?.employeementType || [],
    [positions]
  );
...
export default LayoutHOC(EmploymentType);
