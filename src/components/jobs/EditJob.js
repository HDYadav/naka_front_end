...
import { CREATE_JOB, EDIT_JOB } from "../../utils/constants"; // Update the endpoint for editing a job
...
const EditJob = () => {
  ...
  const { id } = useParams();
  const { data: job, loading, error } = useEditJob(id);
  ...
  useEffect(() => {
    if (job) {
        setInitialValues({
          ...
          jobPosiiton: job.jobPosiiton || "",
          education: job.education || "",
          work_place: job.workPlace || "",
          employeementType: job.employeementType || "",
          skills: job.skills || [],
          description: job.description || "",
        });
      setSelectedDate(job.deadline ? new Date(job.deadline) : null);
    }
  }, [job]);
  ...
  const handleSubmit = async (values, { setSubmitting, setFieldError }) => {
    try {
      ...
      //     // Update the URL for editing the job
      //     method: "PUT", // Use PUT for updating the job
      //     headers: {
                ...
                <div className="flex px-10 font-poppins pt-3 justify-between">
                  ...
                  <div className="flex gap-4">
                    <button
                      ...
                      onClick={() => navigate("/jobs_list/")} // Navigate back to jobs list
                    >
                      CANCEL
                    </button>
                    <button
...
export default LayoutHOC(EditJob);
